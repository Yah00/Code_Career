...
public class CC_35_Depth_B_Tree {
	...
	boolean IsBalanced_method1(Node node){
		if(node == null){
			return true;
		}
		...
		return IsBalanced_method1(node.leftchild) && IsBalanced_method1(node.rightchild);
	}
	
	
	boolean IsBalanced_method2(Node node){
		int depth = 0;
		return sub_method2(node, depth);
	}
	
	public int leftdepth = 0;
	public int rightdepth = 0;
	boolean sub_method2(Node node, int depth){
		if(node == null){
			depth = 0;
			return true;
		}
		...
		//int leftdepth = maxDepth(node.leftchild);
		//int rightdepth = maxDepth(node.rightchild);
		//int leftdepth = 0;------wrong
		//int rightdepth = 0;-----wrong
		if(sub_method2(node.leftchild, leftdepth) && sub_method2(node.rightchild, rightdepth)){
			int temp = leftdepth - rightdepth;
			if(temp <=1 && temp >= -1){
	...
	public static void main(String[] args) {
		...
		System.out.println(bt.IsBalanced_method1(bt.root));
		System.out.println(bt.IsBalanced_method2(bt.root));
